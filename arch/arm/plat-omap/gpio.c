...
static int gpio_irq_type(struct irq_data *data, unsigned type)
{
	...
	if (retval == 0) {
		...
		d->status_use_accessors &= ~IRQ_TYPE_SENSE_MASK;
		d->status_use_accessors |= type;
	}
...
static int gpio_wake_enable(struct irq_data *data, unsigned int enable)
{
...
static void gpio_irq_shutdown(struct irq_data *data)
{
	unsigned int irq = data->irq;
	unsigned int gpio = irq - IH_GPIO_BASE;
	struct gpio_bank *bank = irq_get_chip_data(irq);
...
static void gpio_unmask_irq(struct irq_data *data)
{
	...
	u32 trigger = desc->status_use_accessors & IRQ_TYPE_SENSE_MASK;
