...
static void __init omap_init_hsmmc(struct omap2_hsmmc_info *hsmmcinfo, int ctrl_nr);
...
void __init omap2_hsmmc_init(struct omap2_hsmmc_info *controllers)
{
	...
	omap_ctrl_writel((TPCC_EVT_MUX_SDRXEVT2<<8) | TPCC_EVT_MUX_SDTXEVT2,  TI814X_CONTROL_TPCC_EVT_MUX_40_43);
	for (; controllers && controllers->mmc; controllers++)
...
static void __init omap_init_hsmmc(struct omap2_hsmmc_info *hsmmcinfo, int ctrl_nr)
{
	...
	omap_hsmmc_mux(mmc_data, (hsmmcinfo->mmc-1));
	switch (hsmmcinfo->mmc - 1) {
	case 0:
//=======
#ifdef ACL_MMC2 //#if 1  // hbchen for change order
		if (!cpu_is_ti81xx()) {
