...
#if 0
static void p15_init_ccnt(void)
{
...
static inline unsigned long p15_read_ccnt(void)
{
	unsigned long ccnt;
	__asm__ __volatile__(
		"mrc p15, 0, %0, c9, c13, 0 @ read ccnt"
		: "=r" (ccnt)
		:
		: "cc"
	);
...
// 720MHz with 1/64 count mode = 11.25MHz equiv.
#define t2u(t) (t)/(11250000/1000000)
#endif

#define GET_TIME() ktime_get()
...
module_param_named(addr, buf_addr, ulong, S_IRUSR|S_IWUSR);
module_param_named(size, buf_size, ulong, S_IRUSR|S_IWUSR);
...
dump_trace_entry(struct seq_file *m, int i)
{
	...
	u32 sec = entry->time.tv.sec;
	u32 us = entry->time.tv.nsec / 1000;
...
#ifdef CONFIG_PM_PREVENT_SUSPEND
	extern void pm_suspend_stop(int);
#endif /* CONFIG_PM_PREVENT_SUSPEND */
...
static int __init trace_init(void)
{
	...
	if(!buf_addr) {
		...
		buf_addr = (unsigned long)virt_to_phys(kmalloc(buf_size, GFP_KERNEL));
		printk(KERN_INFO "buffer allocated by kmalloc() = 0x%08lx, size = %lu\n", buf_addr, buf_size);
	}
