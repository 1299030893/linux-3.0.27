...
#if 1// patch for corsica usb-eth adapter, // hbchen
static int cpsw_slave_open(struct cpsw_slave *slave, struct cpsw_priv *priv)
#else
static void cpsw_slave_open(struct cpsw_slave *slave, struct cpsw_priv *priv)
#endif
{
...
#else // hbchen
	ret = cpsw_slave_open(&priv->slaves[priv->emac_port], priv);
	if (ret < 0) {
