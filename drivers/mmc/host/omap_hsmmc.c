...
#include <plat/mmc.h>
#include <plat/cpu.h>
#include <../../../arch/arm/mach-omap2/cm81xx.h> // hbchen

#define ACL_MMC2 1  // hbchen
...
static int omap_hsmmc_card_detect(struct device *dev, int slot)
{
	...
	if (mmc->version != MMC_CTRL_VERSION_2)
	{
		...
		pstate = !gpio_get_value_cansleep(mmc->slots[0].switch_pin);
		return pstate;
	}
	else {
		pstate = 0;
...
// TODO: OLD & NEW parameters are different
...
// TODO： PROTOTYPE be different here, NEW VERSION be kept.
static void omap_hsmmc_do_irq(struct omap_hsmmc_host *host, int status)
{
	struct mmc_data *data;
	int end_cmd = 0, end_trans = 0;
// TODO: OLD VERSION ADD
#if 1
	/* Schedule card detect here ONLY if irq for CD isn't registerted*/
	if ((host->pdata->version == MMC_CTRL_VERSION_2) &&
				((status & CINS) || (status & CREMOVE)))
		omap_hsmmc_cd_handler(host->irq, host);
#endif
	...
	if(status & CREMOVE) {
		dev_dbg(mmc_dev(host->mmc),"Card removed Ignoring card\n");
		if (host->cmd) {
...
static irqreturn_t omap_hsmmc_irq(int irq, void *dev_id)
{
	...
	OMAP_HSMMC_WRITE(host->base, HCTL, OMAP_HSMMC_READ(host->base, HCTL) | SDBP);
...
static void omap_hsmmc_detect(struct work_struct *work)
{
	...
	printk(KERN_INFO "%s:%s %d\n",mmc_hostname(host->mmc),__func__,carddetect);
	if (carddetect){
		if(strncmp("mmc1",mmc_hostname(host->mmc),4)==0){
...
static void omap_hsmmc_request(struct mmc_host *mmc, struct mmc_request *req)
{
	...
	if (mmc_slot(host).card_detect) {
		if(!mmc_slot(host).card_detect(host->dev, host->slot_id)){
			req->cmd->error = -ENOSYS;
			if (req->data)
				req->data->error = -ENOSYS;
			req->cmd->retries = 0;
			mmc_request_done(mmc, req);
			dev_dbg(mmc_dev(host->mmc), "No card. Request ignore\n");
			return;
		}
	}
...
static void omap_hsmmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock) {
		...
		if (dsor > 250)
			dsor = 250;
#endif
	dev_dbg(mmc_dev(host->mmc), "MMCHS_SYSCTL dsor = %d\n",dsor);
	}
...
static int __init omap_hsmmc_probe(struct platform_device *pdev)
{
	...
	if (host->id == 1)
	{
		...
		__raw_readl(TI814X_CM_ALWON_MMCHS_2_CLKCTRL);
	}
#endif
